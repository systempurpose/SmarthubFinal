...
async function renderBsodDiagnosis() {
    ...
    // Always render the page; no early return.
    const html = `
        <div style="margin-bottom:24px;">
    ...
    function renderStateUI(stateData) {
        ...
        // Attach ADB diagnosis button (if present)
    ...
    // ---- ADB-based diagnosis (original logic, using the existing /api/bsod/diagnose) ----
    async function runBsodDiagnosis() {
        ...
        try {
            // Use the original BSOD endpoint (which requires ADB)
    ...
    // Clean up any previous interval (if called again)
    ...
    // Store cleanup so navigation can stop polling
    window._bsodCleanup = () => {
        if (window._bsodPollInterval) {
            clearInterval(window._bsodPollInterval);
            window._bsodPollInterval = null;
